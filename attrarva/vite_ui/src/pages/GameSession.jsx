...
export default function GameSession() {
    ...
    useEffect(() => {
        const init = async () => {
            try { const stats = await getUserStats(); setHearts(stats.hearts); } catch (_) { }
            loadNextPrompt();
        };
        init();
    }, [gameId]);
    ...
    const handleCheck = async () => {
        ...
        try {
            ...
            setFeedback({ success: res.success, reason: res.reason, earned: res.xp_reward, mastery: res.mastery_level || 0 });
    ...
    const handleNext = () => { if (progress >= 100) navigate('/'); else if (!isGameOver) loadNextPrompt(); };
    const handleRefill = async () => { const r = await refillHearts(); setHearts(r.hearts); setIsGameOver(false); loadNextPrompt(); };
    ...
    return (
        <div className="flex flex-col h-full bg-[#F0FDFA]">
            {/* Top bar */}
            <motion.div
            ...
            >
                <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => navigate('/')} className="text-gray-400 hover:text-[#0D9488] transition">
                    <ArrowLeft size={24} />
                </motion.button>
            ...
            <div className="flex-1 overflow-y-auto w-full max-w-2xl mx-auto flex flex-col items-center justify-start p-6 pb-40 mt-10">
                ...
                >
                    ...
                    <motion.div
                        ...
                        className="mb-6 p-6 bg-white border-2 border-[#5EEAD4] rounded-2xl relative shadow-card"
                    >
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="mt-2 flex items-center gap-2">
                                        <span className={`text-xs uppercase font-bold tracking-wider ${feedback.success ? 'text-white/70' : 'text-gray-500'}`}>Mastery</span>
                                        <div className="w-32 h-2 bg-white/30 rounded-full overflow-hidden"><div className="h-full bg-white rounded-full" style={{ width: `${feedback.mastery * 100}%` }} /></div>
                                        <span className={`text-xs font-bold ${feedback.success ? 'text-white/80' : 'text-gray-600'}`}>{Math.round(feedback.mastery * 100)}%</span>
                                    </div>
                                </div>
                            </div>
